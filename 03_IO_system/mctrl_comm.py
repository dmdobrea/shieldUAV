...
class io_expansion_board:
    ...
    def __init__(self, automatic_update, PWM1_min = -1, PWM1_max = 1, PWM2_min = -1, PWM2_max = 1, LED1 = 0, LED2 = 0, LED3 = 0, PWM1 = 0, PWM2 = 0, com_port = 0, baud_rate = 115200):
        '''
        automatic_update    - each set of an LED or servo is automatically followed by an update_state() if set to TRUE
        PWM1/2_min/max      - min max of the servos for automatic clamping
        LED1/2/3            - initial state of the LEDs
        PWM1/2              - initial state of the PWMs
        com_port            - appends this number to the port used e.g. for com_port=3 the /dev/ttyUSB3 is used
        baud_rate           - sets the baud rate for the serial communication
        '''
    ...
    def send_data(self, data):
        '''
        'data'  - list containing only data that can be cast to byte type.
    ...
    def update_state(self):
        '''
        Uses LED1/2/3 and PWM1/2 to create a valid list to send to send_data().
        '''
        to_send = [0x55, 0xAA]
    ...
    def LED_state_set(self, LED1=None, LED2=None, LED3=None):
        ...
        'LED1', 'LED2' or 'LED3' can be:
            ...
            -1 = ON
            -2 = BLINKING
        If not changed state doesn't change.
        '''
        if LED1 == None:
            LED1 = self.LED1
        if LED2 == None:
    ...
    def PWM_value_set(self, PWM1 = None, PWM2 = None):
        '''
        'PWM1' or 'PWM2'    - values in the [-1, 1] interval. They get clamped using the PWM1/2_min/max
        '''
        if PWM1 == None:
            PWM1 = self.PWM1
        if PWM2 == None:
    ...
    def read_buttons(self):
        '''
        Returns two ints each showing how many times each button has been pressed since last time the function was called.
        '''
        button1 = 0
        button2 = 0
...
# +------------------------------------------------------------------+
# | some testing which isn't run if starting in pan_tilt_tracking.py |
# +------------------------------------------------------------------+
