...
class ObjCenter:
	def __init__(self, yoloPath):
		...
		print("[INFO_OD] : YOLO object creation\n")
		
		self.overlay = DpuOverlay("dpu.bit")
		
		self.overlay.load_model(yoloPath)

		anchor_list  = [18, 17, 36, 30, 63, 44, 86, 82, 150, 47, 164, 129] 	# for YOLOv3-tiny
				
		anchor_float = [float(x) for x in anchor_list]
		self.anchors = np.array(anchor_float).reshape(-1, 2)
		
		classes_path = "/home/ubuntu/ros2_kamikazeUAV_ws/src/kamikaze_uav/kamikaze_uav/voc_classes.txt"
		...
		print("[INFO_OD] : YOLO object creation end\n")
		
	def __del__(self):
		print("[INFO_OD] : YOLO object destruction\n\n")
		del self.overlay
		del self.dpu		
